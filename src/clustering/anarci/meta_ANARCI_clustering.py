...
# moving away from this because it contains the same final merging step as ANARCI_clustering.py which causes bad clusterings
# keep for now because I'm still testing the alternative
...
parser.add_argument('--max_len_diff',type=int,nargs=2,default=[6,6], help='Inclusive bounds for the range of values of maximum allowed difference in CDR length.')
...
parser.add_argument('--narrow_mmseqs_min_id',type=float,nargs=2,default=[0.75,0.95],help='Inclusinve bounds for the range of minimum sequence identity values allowed for final narrow mmseqs clustering.')
parser.add_argument('--narrow_mmseqs_min_id_N',type=int,default=3,help='Number of values to test between the narrow_mmseqs_min_id bounds.')

parser.add_argument('--meta_min_id',type=float,default=0.25,help='Minimum fraction of agreeing clusterings to group two VHHs. Clustering distance threshhold = 1 - meta_min_id')
parser.add_argument('--meta_method',type=str,default='average',help='Linkage method to use for meta clustering (can be single, average, or complete).')
...
final_min_id_vals = np.linspace(*args.narrow_mmseqs_min_id,args.narrow_mmseqs_min_id_N)
            ...
            for cdr_idx,dist_params in enumerate(cdr3_dist_combos):
                ...
                if len(mmseqs_group) == 1:
                    ...
                    for clust_idx,_ in enumerate(weighted_clustering_combos):
                        vhh.loc[mmseqs_group.index,f'weighted_id_{rec_idx}_{cdr_idx}_{clust_idx}'] = min_id[(rec_idx,cdr_idx,clust_idx)] #use min_id as usual
                        # print(f'(skip) SCOPer: {g}/{len(vhh["scoper_group"].unique())}, rec_idx: {rec_idx}/{len(recursive_min_id_vals)}, mmseqs: {mm_id}/{len(scoper_group["mmseqs_id"].unique())}, '+\
                        #       f'cdr_idx: {cdr_idx}/{len(cdr3_dist_combos)}, clust_idx: {clust_idx}/{len(weighted_clustering_combos)}',flush=True)
                        # print(mmseqs_group.index,'\n',flush=True)
                ...
                #actually do clustering for groups with >1 member
                    ...
                    dists = np.array(pool.starmap(ANARCI_dist_row,((CDR_list1,
                                                                    CDR_list_list,
                                                                    [1,1,dist_params[0]],
                                                                    [dist_params[1]]*3) for CDR_list1 in CDR_list_list),
                                                  chunksize=10))
                    ...
                    for clust_idx,clust_params in enumerate(weighted_clustering_combos):
                        ...
                        vhh.loc[mmseqs_group.index,f'weighted_id_{rec_idx}_{cdr_idx}_{clust_idx}'] = labels
                        # print(f'(cluster) SCOPer: {g}/{len(vhh["scoper_group"].unique())}, rec_idx: {rec_idx}/{len(recursive_min_id_vals)}, mmseqs: {mm_id}/{len(scoper_group["mmseqs_id"].unique())}, '+\
                        #       f'cdr_idx: {cdr_idx}/{len(cdr3_dist_combos)}, clust_idx: {clust_idx}/{len(weighted_clustering_combos)}',flush=True)
                        # print(mmseqs_group.index,'\n',flush=True)
            
                #update min_id for all clustering variations
                for clust_idx,_ in enumerate(weighted_clustering_combos):
                    min_id[(rec_idx,cdr_idx,clust_idx)] = vhh[f'weighted_id_{rec_idx}_{cdr_idx}_{clust_idx}'].max() + 1
...
print(f'Finished within-SCOPer clustering.', flush=True)

# vhh.to_csv(args.out_file)
    
#############
## final cluster merging for all clusterings performed above
#############
...
id_cols = [col for col in vhh.columns if 'weighted_id' in col]
vhh = vhh.astype(dtype={col:int for col in id_cols})
...
for col in tqdm(id_cols):
    ...
    rep_seqs = vhh.sort_values(by='VHH_duplicate_count',ascending=False)[[col,'VHH']].groupby(col).first()
    
    #write fasta file
    with open(f'{tmp_dir}/reps.fasta','w') as f:
        for i,row in rep_seqs.iterrows():
            f.write(f">{i}\n{row['VHH']}\n") #Index here is the weighted_id
    #try each requested merge min_id
    for merge_idx,merge_min_id in enumerate(final_min_id_vals):
        ...
        result = subprocess.run(['/cluster/tufts/cowenlab/emosel01/condaenv/vhh/bin/mmseqs', 'easy-cluster', f'{tmp_dir}/reps.fasta', f'{tmp_dir}/clust', f'{tmp_dir}/tmp',
                                 ...
                                 '-c', str(max(0,(merge_min_id - 0.2))), #set this lower than seq-id cutoff so it doesn't have an impact
                                 '--min-seq-id', str(merge_min_id), #only cluster seqs more similar than user input
        ...
        #read in clusters
        final_clusters = pd.read_csv(f'{tmp_dir}/clust_cluster.tsv',names=[col.replace('weighted','clone')+f'_{merge_idx}',col],sep='\t')
        
        #merge on final cluster info
        vhh = vhh.merge(final_clusters,on=col,how='left')
        
    #clean up tmp_dir
    for f in glob(f'{tmp_dir}/*'):
        subprocess.run(['rm', '-r', f])
        
    #delete weighted_id col since it is no longer needed
    del vhh[col]
...
#run clustering
links = linkage(group_reps[id_cols].values,method=args.meta_method,metric='hamming')
labels = fcluster(links,criterion='distance',t=1-args.meta_min_id)
group_reps.loc[:,'meta_clone_id'] = labels

#add results to vhh dataframe
print(f'{len(vhh)} VHHs before merge.',flush=True)
vhh = vhh.merge(group_reps[['fine_clone_id','meta_clone_id']],on='fine_clone_id',how='outer')
...
if args.keep_intermediates:
    data = data.merge(vhh[['VHH','CDR1','CDR2','CDR3']+id_cols+['meta_clone_id']],on='VHH',how='outer')
else:
    data = data.merge(vhh[['VHH','CDR1','CDR2','CDR3','meta_clone_id']],on='VHH',how='outer')
print(f'{len(data)} sequences after merge.',flush=True)
print(f'{len(data["meta_clone_id"].unique())} families found.',flush=True)

del data['X'] #remove X column that gets created by SCOPer
